...
module.exports = function replaceCollection (req, res) {
  ...
  async.auto({
    ...
    notificationsForExistingParentsOfReplacementChildren: function(cb) {
      ...
      criteria.select = [attr.via];
      relatedModel.find(criteria).exec(function(err, childRecords) {
        _.each(childRecords, function(childRecord) {
    ...
    notificationsForExistingChildrenOfParent: function(cb) {
      ...
      relatedModel.find(criteria, function(err, childRecords) {
        _.each(childRecords, function(childRecord) {
    ...
    // ```
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Model.findOne(parentPk).populate(relation).exec(function(err, parentRecord) {
      var currentChildPks = _.map(parentRecord[relation], relatedModel.primaryKey);
      currentChildPks.forEach(function(pk) {
        if (childPks.indexOf(pk) === -1) {
          parentRecord[relation].remove(pk);
        }
      });
      childPks.forEach(function(pk) {
        if (currentChildPks.indexOf(pk) === -1) {
          parentRecord[relation].add(pk);
        }
      });
